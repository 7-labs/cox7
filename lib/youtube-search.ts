  ...
  try {
    ...
    if (!searchResponse.ok) {
      const body = createFallbackResponse({
        ...
        league: params.league,
        type: params.type,
        note: `YouTube search returned ${searchResponse.status}; showing trusted example videos instead.`,
    ...
    if (searchCandidates.length === 0) {
      const body = createFallbackResponse({
        ...
        league: params.league,
        type: params.type,
        note: "No trusted preview candidates matched this query yet; showing trusted example videos instead.",
    ...
    if (!videosResponse.ok) {
      const body = createFallbackResponse({
        ...
        league: params.league,
        type: params.type,
        note: `YouTube video verification returned ${videosResponse.status}; showing trusted example videos instead.`,
        fallbackReason: "upstream_http_error",
        lastChecked: params.lastChecked,
        rateLimit: params.rateLimit,
    ...
    if (verifiedCandidates.length === 0) {
      const body = createFallbackResponse({
        ...
        league: params.league,
        type: params.type,
        note: "Trusted candidates were found, but none passed public embeddable verification; showing trusted example videos instead.",
    ...
    const body = createFallbackResponse({
      ...
      league: params.league,
      type: params.type,
      note: "YouTube API request failed; showing trusted example videos instead.",
...
export async function executeSearchRequest(params: SearchRequest): Promise<SearchExecutionResult> {
  ...
  if (inventory.source === "seed") {
    const body = createFallbackResponse({
      ...
      league: params.league,
      type: params.type,
      note: "The live preview database is unavailable right now; showing trusted example videos instead.",
  ...
  if (shouldUseLiveFallback() && params.apiKey) {
    ...
    if (rateLimit.limited) {
      const body = createFallbackResponse({
        ...
        league: params.league,
        type: params.type,
        note: "Search is temporarily rate limited; showing trusted example videos instead.",
        source: "rate-limited",
  ...
  const body = createFallbackResponse({
    ...
    league: params.league,
    type: params.type,
    note: "No matching inventory rows were found yet; showing trusted example videos instead.",
